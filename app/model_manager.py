...
class ModelManager:
    ...
    def get_prediction(self, datasets, target_set, curiosity_weight, target_items, found_items_with_ratio, previous_set_states = None, previous_operation_states = None):
        state_encoder = StateEncoder(self.pipeline, target_items=target_items, found_items_with_ratio=found_items_with_ratio)
        ...
        if previous_set_states == None:
            previous_set_states = [[-1]*len(state_encoder.set_description)*self.pipeline.discrete_categories_count]*10

        new_set_states = previous_set_states
        new_set_states.pop(0)
        new_set_states.append(set_state)
        
        set_probs = set_actor.predict(np.array(new_set_states).reshape((1, 10, len(state_encoder.set_description)*self.pipeline.discrete_categories_count)))[0]
        print(set_probs)
        set_probs = self.action_manager.fix_possible_set_action_probs(datasets, set_probs)
        print(set_probs)
        set_action = np.random.choice(self.action_manager.set_action_dim, p=set_probs)
        operation_state, dummy_reward = state_encoder.encode_dataset(datasets[set_action], get_reward=False)

        if previous_operation_states == None:
            previous_operation_states = [[-1]*len(state_encoder.set_description)]*10

        new_operation_states = previous_operation_states
        new_operation_states.pop(0)
        new_operation_states.append(operation_state)
        
        operation_probs = operation_actor.predict(np.array(new_operation_states).reshape((1, 10, len(state_encoder.set_description))))[0]
        print(operation_probs)
        operation_probs = self.action_manager.fix_possible_operation_action_probs(datasets[set_action], operation_probs)
        print(operation_probs)
        operation_action = np.random.choice(self.action_manager.operation_action_dim, p=operation_probs)
        ...
        return {
            ...
            "foundItemsWithRatio": found_items_with_ratio,
